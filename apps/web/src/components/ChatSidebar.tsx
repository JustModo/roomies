...
export const ChatSidebar: React.FC = () => {
  ...
  const toastsUI = !isOpen && toasts.length > 0 ? (
    <div className="hidden lg:flex fixed bottom-18 right-4 w-[300px] flex-col justify-end pointer-events-none z-45">
      <div
      ...
      >
        {toasts.map((toast, index) => {
          ...
          return (
            <div
              key={toast.id}
              className={`transition-all duration-300 text-13 text-paper leading-normal break-words ${paddingClass} ${toast.isExiting ? 'opacity-0 scale-95' : 'opacity-100 scale-100'
                }`}
            >
              {toast.isSystem ? (
  ...
  return (
    <>
      {toastsUI}
      <div className={`relative flex-1 lg:fixed lg:top-0 lg:right-0 lg:w-[360px] lg:h-screen bg-void lg:border-l border-t lg:border-t-0 border-ash/10 flex-col z-45 shadow-2xl w-full ${!isOpen ? 'flex lg:hidden' : 'flex'}`}>
        {/* Header */}
