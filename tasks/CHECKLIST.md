...
- [x] **Adaptive Streaming** — was already substantially implemented (multi-variant `master.m3u8`, `hls.js` ABR on the frontend); the fast-start pre-warming above closes the remaining gap (on-demand variant spawn causing a stall on first play/seek).
  - [x] Add configurable FFmpeg presets (ultrafast/veryfast/fast/medium/slow) and hwaccel mode (auto/cpu), read from `roomies.conf` (`FFMPEG_PRESET`/`HWACCEL_MODE`, parsed by `packages/config`, consumed by `packages/transcoding/src/settings.ts` via `initTranscodeSettings()`) `[LOG:2026-07-08 Transcode Settings Moved to Config File]`. No longer DB-backed or live-editable — the Admin Overlay SETTINGS tab and `GET`/`PATCH /api/settings/transcode` were removed; change the `.conf` file and restart to take effect.
